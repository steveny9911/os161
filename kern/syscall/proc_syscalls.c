 ...
 * @return does not return on success, else error code
 */
int sys_execv(const char *program, char **args)
{
    ...
    for (int i = 0; i < argc; i++) {
        ...
        size_t strlen = arglen * sizeof(char);   // length to be copied
        argbuf[i] = (char *)kmalloc(strlen);     // malloc space on kernel
        result = copyin((const_userptr_t)args[i], argbuf[i], strlen);
        if (result) {
            kfree(progname);
            for (int j = 0; j < argc; j++) {
                kfree(argbuf[j]);
    ...
    //
    // we will keep track of argument address (in user space) using argaddr
    //
    vaddr_t *argaddr = (vaddr_t *)kmalloc((argc + 1) * sizeof(vaddr_t));

    // copy out argument strings
    for (int i = 0; i < argc; i++) {
        size_t arglen = strlen(argbuf[i]) + 1;                                 // length of each argument
        size_t strlen = ROUNDUP(arglen, 4) * sizeof(char);                     // apply alignment for each string
        stackptr -= strlen;                                                    // decrement stackpointer for string
        result = copyout((void *)argbuf[i], (userptr_t)stackptr, strlen);      // copy string onto location of stackpointer
        if (result)
        {
        ...
        argaddr[i] = stackptr;  // record this location! this particular string's address
    }

    argaddr[argc] = (vaddr_t)NULL;
    ...
    // but it doesn't really matter since we kept their addresses in argaddr
    //
    for (int i = argc; i >= 0; i--) {
        size_t addrlen = sizeof(vaddr_t);                                        // vaddr_t is u32 --- which is 4 bytes
        stackptr -= addrlen;                                                    // decrement stackpointer
        result = copyout((void *)&argaddr[i], (userptr_t)stackptr, addrlen);   // copy out the address (which we recorded earlier) onto location of the stackpointer
        if (result) {
            for (int j = 0; j < argc; j++) kfree(argbuf[j]);
            kfree(argbuf);
            kfree(argaddr);
